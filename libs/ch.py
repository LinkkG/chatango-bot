...
# Original Author: Lumirayz/Lumz <lumirayz@gmail.com> 1068
...
class Room:
  ...
  def _rcmd_getannc(self, args):
    ...
    self._announcement[0] = int(args[0])
    self._announcement[1] = int(args[3])
    self._announcement[2] = ":".join(args[4:])
  ...
  def _rcmd_participant(self, args):
    ...
    if args[0] == "0": #leave
      ...
      if user in self._userlist: self._userlist.remove(user)
  ...
  def message(self, msg, html = False, *, channels = None, badge = None):
    ...
    if isinstance(badge, int):
        ...
        _channel |= Channels.get(badge.lower(), 0)
    if msg==None:
      return
    if "\n" in msg:
        msg = msg.replace("\n", "\r")
    ...
    if len(msg) > self.mgr._maxLength:
      if self.mgr._tooBigMessage == BigMessage_Cut:
        ...
        while len(msg) > 0:
          ...
          self.message(sect, html = html, channels = _channel)
      return
  ...
  def setAnnouncement(self, enabled=False, time=0, announcement=None):
    """
    Change group announcement or just enable/disable with first argument
    @param enabled: announcement status, default 0/False for disabled, 1/True for enabled, 3 for enabled with bg
    @type enabled: int
    @param time: seconds between each announcement
    @type time: int
    @param announcement: message to send
    @type announcement: str
    """
    if self.getLevel(self.user)>0:
      self._sendCommand('updateannouncement', int(enabled), time or 60, announcement or self._announcement[2])
      return True
    return False
  ...
  def clearall(self):
    ...
    if self.owner == self.mgr.user:
      self._sendCommnad("clearall")
    elif self.getLevel(self.mgr.user):
      for user in set(x.user for x in self._msgs.values()):
        msg = self.getLastMessage(user)
